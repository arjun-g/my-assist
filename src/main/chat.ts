...
ipcMain.on('chat-initiate', async (event, arg) => {
  ...
  await settings.set('gemini.apikey', 'AIzaSyDDpCv6U1jD4NDOy3rrbDf4HZgZY-SDKDc');

});

ipcMain.on('chat-list', async (event, arg) => {
  const messages = (await chatDB.listItems<AIMessage>()).filter(msg => {
    return ['user', 'model'].includes(msg.metadata.role) && !msg.metadata.content[0].functionCall;
  }).map(msg => {
    return {
      id: msg.id,
      role: msg.metadata.role,
...
ipcMain.on('chat-message', async (event, arg) => {
  ...
  const vector = await AI.embedText(JSON.stringify({ text: message, datetime: new Date().toISOString() }));
  ...
  for(const msg of reply){
    ...
    if((msg.role === 'model' || msg.role === 'user') && !msg.content.find(content => content.functionCall)){
      BrowserWindow.getAllWindows()[0].webContents.send('chat-new-message', {
        id: newMessage.id,
        role: newMessage.metadata.role,
        content: newMessage.metadata.content,
    ...
    const vector = await AI.embedText(JSON.stringify({ text: msg.content, datetime: msg.timestamp }));
    newMessage.vector = vector;
    await chatDB.insertItem(newMessage);
  }
});
